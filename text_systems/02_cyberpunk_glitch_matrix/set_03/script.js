...
textInput.addEventListener('input', (e) => {
  heroText.textContent = e.target.value || 'qUaNtUm_cIpHeR';
});
...
copyCssBtn.addEventListener('click', () => {
  const css = `font-family: 'Major Mono Display', sans-serif;
font-weight: 400;
letter-spacing: 0.02em;
text-transform: none;
font-size: clamp(2.5rem, 6vw, 4.8rem);
color: #38bdf8; text-shadow: 0 0 20px rgba(56,189,248,0.5);`;
...
copyPromptBtn.addEventListener('click', () => {
  const p = `Use the 'Major Mono Decryptor' typography treatment from 'Cyberpunk Glitch Matrix':
- Font Family: 'Major Mono Display', sans-serif (Google Fonts)
- Weight: 400
- Tracking / Spacing: 0.02em
- Text Transform: none
- Custom Styling: color: #38bdf8; text-shadow: 0 0 20px rgba(56,189,248,0.5);
