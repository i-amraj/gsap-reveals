...
textInput.addEventListener('input', (e) => {
  heroText.textContent = e.target.value || 'ACCESS GRANTED';
});
...
copyCssBtn.addEventListener('click', () => {
  const css = `font-family: 'Source Code Pro', sans-serif;
font-weight: 800;
letter-spacing: 0.05em;
text-transform: uppercase;
font-size: clamp(2.5rem, 6vw, 4.8rem);
color: #10b981; text-shadow: 0 0 25px rgba(16,185,129,0.7);`;
...
copyPromptBtn.addEventListener('click', () => {
  const p = `Use the 'Source Code Cyber Alert' typography treatment from 'Cyberpunk Glitch Matrix':
- Font Family: 'Source Code Pro', sans-serif (Google Fonts)
- Weight: 800
- Tracking / Spacing: 0.05em
- Text Transform: uppercase
- Custom Styling: color: #10b981; text-shadow: 0 0 25px rgba(16,185,129,0.7);
