...
textInput.addEventListener('input', (e) => {
  heroText.textContent = e.target.value || 'const AI = new World();';
});
...
copyCssBtn.addEventListener('click', () => {
  const css = `font-family: 'Fira Code', sans-serif;
font-weight: 700;
letter-spacing: -0.01em;
text-transform: none;
font-size: clamp(2.5rem, 6vw, 4.8rem);
color: #f43f5e; text-shadow: 0 0 15px rgba(244,63,94,0.5);`;
...
copyPromptBtn.addEventListener('click', () => {
  const p = `Use the 'Fira Code Terminal Glyph' typography treatment from 'Cyberpunk Glitch Matrix':
- Font Family: 'Fira Code', sans-serif (Google Fonts)
- Weight: 700
- Tracking / Spacing: -0.01em
- Text Transform: none
- Custom Styling: color: #f43f5e; text-shadow: 0 0 15px rgba(244,63,94,0.5);
