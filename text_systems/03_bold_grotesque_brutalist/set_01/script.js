...
textInput.addEventListener('input', (e) => {
  heroText.textContent = e.target.value || 'MASSIVE FORCE';
});
...
color: #ffffff; text-shadow: 0 10px 40px rgba(0,0,0,0.8);`;
...
copyPromptBtn.addEventListener('click', () => {
  const p = `Use the 'Syne Ultra Heavy' typography treatment from 'Bold Grotesque Brutalist':
- Font Family: 'Syne', sans-serif (Google Fonts)
- Weight: 800
- Tracking / Spacing: -0.04em
- Text Transform: uppercase
- Custom Styling: color: #ffffff; text-shadow: 0 10px 40px rgba(0,0,0,0.8);
