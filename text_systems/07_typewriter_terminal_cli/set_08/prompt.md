# AI Typography Prompt: NASA Telemetry Terminal (Typewriter Terminal CLI)

```markdown
Apply the 'NASA Telemetry Terminal' typography style from the Typewriter Terminal CLI system to all display headings and hero banners.

Typographic Specifications:
- Google Font: 'Inconsolata'
- Font Weight: 900
- Letter Spacing: 0.04em
- Text Transform: uppercase
- Fluid Clamp Sizing: clamp(2.5rem, 6vw, 4.8rem)
- Custom Stylings:
  color: #38bdf8;

Guidelines:
- Pair with 'Plus Jakarta Sans' for body paragraphs and 'JetBrains Mono' for technical tags.
- Apply a responsive line-height of 1.1 for display headlines.
- Maintain high contrast against dark or light canvas backgrounds.
```
